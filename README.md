...
**许可范围**（LICENSE 正文里不写这些，因为 GitHub 需要文件逐字匹配 MIT 才能正确识别）：

| | |
|---|---|
| ✅ 涵盖 | 全部代码、文档，以及仓库里**训练好的模型权重** |
| ❌ 不涵盖 | 训练用的刘慈欣原文 —— 它不属于本仓库，**MIT 也无法授予你任何与它有关的权利** |

换句话说：你可以把这块板子和它的模型拿去做任何事；但你不能因此认为自己获得了那些
小说的任何授权。原文不在这里，也不要跟着本仓库一起分发。
